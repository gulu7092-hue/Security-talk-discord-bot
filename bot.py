...
# ─── OWNER CONFIG ─────────────────────────────────────────────────────────────
OWNER_ID = 1503797215733481552  # Ton ID créateur
approved_owners: set[int] = set()  # Owners approuvés
...
def is_owner(user_id: int) -> bool:
 """Vérifie si l'utilisateur est owner (créateur ou approuvé)"""
 return user_id == OWNER_ID or user_id in approved_owners
...
async def cmd_addowner(ctx, member: discord.Member):
 """Ajouter un owner (créateur seulement)"""
 if not is_creator(ctx.author.id):
 await ctx.send("❌ Seul le créateur peut ajouter des owners !")
 return
 
 if member.id in approved_owners:
 await ctx.send(f"⚠️ {member.mention} est déjà owner !")
 return
 
 approved_owners.add(member.id)
 await ctx.send(f"✅ {member.mention} a été ajouté en tant qu'owner !")

@bot.command(name="removeowner")
async def cmd_removeowner(ctx, member: discord.Member):
 """Retirer un owner (créateur seulement)"""
 if not is_creator(ctx.author.id):
 await ctx.send("❌ Seul le créateur peut retirer des owners !")
 return
 
 if member.id not in approved_owners:
 await ctx.send(f"⚠️ {member.mention} n'est pas owner !")
 return
 
 approved_owners.discard(member.id)
 await ctx.send(f"✅ {member.mention} a été retiré des owners !")

@bot.command(name="giverole")
async def cmd_giverole(ctx, member: discord.Member, *, role_name: str):
 """Donner un rôle à quelqu'un (créateur + owners approuvés)"""
 if not is_owner(ctx.author.id):
 await ctx.send("❌ Seul le créateur et les owners peuvent utiliser cette commande !")
 return
...
async def cmd_servers(ctx):
 """Afficher tous les serveurs du bot avec option de quitter"""
 if not is_owner(ctx.author.id):
 await ctx.send("❌ Seul le créateur et les owners peuvent utiliser cette commande !")
 return
 ...
 # Créer un embed pour chaque serveur
 for guild in guilds:
 embed = discord.Embed(
 ...
 try:
 await guild.leave()
 await interaction.response.send_message(f"✅ Le bot a quitté **{guild.name}** !", ephemeral=True)
 except Exception as e:
 await interaction.response.send_message(f"❌ Erreur : {e}", ephemeral=True)
 
 button.callback = leave_callback
